...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    $update_query = $conn->prepare("UPDATE `pool_booking` SET `status` = ? WHERE `booking_id` = ?");
                ...
                <div class="table-responsive">
                    <table id="table" class="display" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            <?php
                            $conn = new mysqli("localhost", "root", "", "ltl") or die(mysqli_error());
                            $query = $conn->query("SELECT * FROM `pool_booking` ORDER BY `booking_id` DESC") or die(mysqli_error());
                            while ($fetch = $query->fetch_array()) {
                                $id = $fetch['booking_id'];
                                $q = $conn->query("SELECT COUNT(*) as total FROM `pool_booking` where `booking_id` = '$id'") or die(mysqli_error());
                                $f = $q->fetch_array();
                            ?>
                                <tr>
                                    ...
                                    <td><?php echo $fetch['number_of_pools'] ?></td>
                                    ...
                                    <td>
                                        <a href="update_poolorder.php?id=<?php echo $fetch['booking_id']; ?>" class="btn btn-primary"> UPDATE</a>
                                        <a href="try.php?booking_id=<?php echo $fetch['booking_id'] ?>&name=<?php echo $fetch['user_email'] ?>" class="btn btn-sm btn-info">
